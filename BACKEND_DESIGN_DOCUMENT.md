...
## BACKEND SIMPLE UI TESTKIT
Swagger URL: <https://defi-yapi.stocksuite.app/docs>

### How to Use (Quick)
1. Open `POST /v1/documents/upload-and-parse`.
2. Click **Try it out**.
3. Upload your document file and execute.
4. Copy `job.id` from the response JSON.

Important:
- `job_id: 3fa85f64-5717-4562-b3fc-2c963f66afa6` is only a Swagger sample value.
- Always use the real `job.id` returned by your upload response.

### View Parsed Output
1. Open `GET /v1/jobs/{job_id}/artifacts`.
2. Click **Try it out**.
3. Paste the real `job.id`.
4. Execute and inspect parsed JSON in artifacts.
...
Recommended flow:
  ...
  1. User uploads file in Vite.
  2. Vite sends file to python_pipeline backend.
  3. Backend parses, normalizes, validates, stores rows in DB.
  4. Vite fetches rows from backend and shows review UI.
  5. User approves/rejects/edits in Vite.
  6. Vite sends those actions back to backend.
  7. Backend updates DB + logs review events.
  
  Two valid options:

  1. Same backend (python_pipeline) handles everything

  - simplest and safest
  - one source of truth

  2. Split backend (your plan)

  - python_pipeline: extraction/parsing only
  - review backend (friend): review actions + DB persistence/workflow
