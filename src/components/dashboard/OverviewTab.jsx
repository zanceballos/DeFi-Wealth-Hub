...
const INSIGHT_TONE_MAP = {
  red: 'border-red-500 bg-red-50/70 text-red-800',
  amber: 'border-amber-400 bg-amber-50/70 text-amber-800',
  ...
  return (
    <div className="space-y-6">
      <header className="px-1">
        <h1 className="text-2xl font-bold tracking-tight text-slate-900 sm:text-3xl">Good morning, {userProfile.name.split(' ')[0]} 👋</h1>
          ...
          return (
            <article key={pillar.name} className={`${CARD_CLASS} p-5`}>
              ...
              <div className="mt-4 h-2 overflow-hidden rounded-full bg-slate-200">
                <div className={`h-full rounded-full ${pillar.colorClass}`} style={{ width: `${pillar.score}%` }} />
              </div>
      ...
      <div className="space-y-3">
        {overviewInsights.map((insight) => (
          <article
            key={insight.text}
            className={`${CARD_CLASS} border-l-4 p-4 ${INSIGHT_TONE_MAP[insight.tone]}`}
          >
