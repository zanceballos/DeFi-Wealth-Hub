...
const VIA_BADGE = {
  ...
  email: { label: "Gmail", className: "bg-blue-100 text-blue-700" },
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white/70 backdrop-blur-xl border border-white/60 rounded-3xl shadow-xl overflow-hidden">
        <div
          ...
          style={paginated.length > 0 ? { maxHeight: "600px" } : undefined}
        >
          <table className="min-w-[600px] w-full">
            <thead className="sticky top-0 bg-white z-10">
              <tr className="border-b border-gray-100">
                ...
                <th className="hidden sm:table-cell text-left px-4 py-3 text-xs text-gray-400 font-medium">
                  Time
                </th>
                <th className="text-left px-4 py-3 text-xs text-gray-400 font-medium">
                  Merchant
                </th>
                <th className="hidden md:table-cell text-left px-4 py-3 text-xs text-gray-400 font-medium">
                  From
                </th>
                <th className="hidden md:table-cell text-left px-4 py-3 text-xs text-gray-400 font-medium">
                  To
                ...
                <th className="hidden lg:table-cell text-center px-4 py-3 text-xs text-gray-400 font-medium">
                  Via
                </th>
                <th className="text-center px-3 py-3 text-xs text-gray-400 font-medium">
                  Actions
                </th>
              </tr>
            </thead>
            <tbody>
              {paginated.length === 0 ? (
                ...
                paginated.map((tx, i) => {
                  ...
                  const viaBadge =
                    VIA_BADGE[tx.dataSource] ?? VIA_BADGE.statement;

                  if (isEditing) {
                    const fromVal =
                      tx.direction === "credit"
                        ? (tx.counterparty ?? "—")
                        : tx.accountRef && tx.accountRef !== "—"
                          ? tx.accountRef
                          : (tx.merchant ?? "—");
                    const toVal =
                      tx.direction === "credit"
                    ...
                    return (
                      ...
                      >
                        <td className="px-5 py-2 text-sm text-gray-500 whitespace-nowrap">
                          {tx.date}
                        </td>
                        <td className="hidden sm:table-cell px-4 py-2 text-sm text-gray-400">
                          {tx.time || "—"}
                        </td>
                        ...
                        <td className="px-4 py-2">
                          <CategoryBadge
                            ...
                            onCategoryChange={(cat) =>
                              setEditValues((v) => ({ ...v, category: cat }))
                            }
                          />
                        </td>
                        <td className="hidden lg:table-cell px-4 py-2 text-center">
                          <span
                            className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-[10px] font-semibold ${viaBadge.className}`}
                          >
                            {viaBadge.label}
                          </span>
                        </td>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="hidden sm:table-cell px-4 py-3 text-sm text-gray-400 whitespace-nowrap">
                        {tx.time || "—"}
                      </td>
                      ...
                      <td className="hidden md:table-cell px-4 py-3 text-sm text-gray-600">
                        <span
                          className="truncate max-w-35 block"
                          title={fromVal}
                        >
                          {fromVal !== "—" ? (
                            fromVal
                      ...
                      <td className="hidden md:table-cell px-4 py-3 text-sm text-gray-600">
                        <span className="truncate max-w-35 block" title={toVal}>
                          {toVal !== "—" ? (
                            toVal
                          ) : (
                            <span className="text-gray-300">—</span>
