...
function stripHtml(html) {
  return html
    .replace(/<br\s*\/?>/gi, "\n")
    .replace(/<\/(?:p|div|tr|td|li|h[1-6]|blockquote)>/gi, "\n")
    .replace(/<[^>]+>/g, " ")
    .replace(/[ \t]+/g, " ")
    .replace(/\n[ \t]*/g, "\n")
    .replace(/\n{3,}/g, "\n\n")
...
function extractBody(payload) {
  ...
  if (payload.body?.data) {
    ...
    // Strip HTML tags if this part is HTML (or if the decoded text looks like HTML)
...
// NOTE: No "subject:" prefix — searches full message so emails with subjects like
// "DBS Digibank Alert" or "Your payment is successful" are also matched.
const GMAIL_QUERY =
  '(transaction OR payment OR receipt OR "card ending" OR debit OR credit OR transfer OR "transaction alert" OR "payment successful" OR DBS OR OCBC OR MariBank OR GrabPay) newer_than:30d';
...
export async function syncGmailTransactions(uid) {
  ...
  const listRes = await gmailFetch(
    `/users/me/messages?q=${encodeURIComponent(GMAIL_QUERY)}&maxResults=50`,
  );
  const messageIds = (listRes.messages || []).map((m) => m.id);
  console.log(
    "[GmailSync] Gmail returned",
    messageIds.length,
  ...
  try {
    ...
    console.error(
      "[GmailSync] ❌ Firestore dedup query failed:",
      err.message || err,
    );
    console.error(
      "[GmailSync] This is likely a Firestore security rules issue. Skipping dedup and processing all messages.",
    );
  ...
  console.log(
    "[GmailSync] New (not yet processed):",
    newIds.length,
    "of",
    messageIds.length,
  );
  ...
  for (const msgId of newIds) {
    try {
      ...
      if (!parsed) {
        console.log(
          "[GmailSync] ⏭ Not a transaction email, skipping:",
          email.subject,
        );
        continue;
      }
      ...
      await addDoc(txCol, {
        ...
        receivedAt: parsed.emailDate
          ? new Date(parsed.emailDate)
          : serverTimestamp(),
