...
import {
  Shield, FileText, FileSpreadsheet, Folder, Coins, Upload,
  CheckCircle2, XCircle, Lock, Key, Eye, RefreshCw, ChevronDown,
  ChevronUp, LifeBuoy, Trash2, ArrowRight, Loader2, AlertTriangle,
  Mail, Calendar, RotateCcw, Unlink,
} from "lucide-react";
...
export default function Privacy() {
  const {
    grouped, loading, deleting, totalFiles, totalTransactions, totalNetWorth,
    deleteStatement, deleteAllStatements, recomputing, emailTxCount,
    emailTxByDate, getDeleteSummary, deleteAllEmailTx, deleteEmailTxByDate, deleteAll,
  } = usePrivacy();
      ...
      <main className="max-w-4xl mx-auto px-6 py-10 space-y-10">
        ...
        <div className="overflow-x-auto pb-1 -mx-1 px-1">
          <div className="flex gap-4 sm:grid sm:grid-cols-3 min-w-[480px] sm:min-w-0">
            {[
              ...
              <div
                key={label}
                className="flex-1 bg-white border border-gray-100 rounded-2xl p-5 text-center"
              >
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center gap-2 mt-1 flex-wrap">
                                  ...
                                  {stmt.net_worth_contribution > 0 && (
                                    <span className="text-xs text-gray-400">{formatSGD(stmt.net_worth_contribution)}</span>
                                  )}
                                </div>
                              </div>
                              <button
        ...
        <section>
          ...
          {emailTxByDate.length === 0 ? (
            <div className="bg-white border border-gray-100 rounded-2xl p-10 text-center">
              ...
              <p className="text-sm font-semibold text-gray-500">No email transactions yet</p>
              <p className="text-xs text-gray-400 mt-1">
                Link your Gmail from the Budgeting tab to pull transaction emails automatically.
              </p>
            </div>
          ) : (
            <div className="space-y-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-gray-900 text-sm">
                      {emailTxCount} email transaction{emailTxCount !== 1 ? "s" : ""} across{" "}
                      {emailTxByDate.length} date{emailTxByDate.length !== 1 ? "s" : ""}
                    </p>
                    <p className="text-xs text-gray-400 mt-0.5">Parsed from linked Gmail account</p>
                  </div>
                </div>
                <button
              ...
              {emailTxByDate.map(([dateKey, txs]) => {
                ...
                return (
                  <div key={dateKey} className="bg-white border border-gray-100 rounded-2xl overflow-hidden">
                    <button
                      onClick={() => setOpenType(isOpen ? null : `email-${dateKey}`)}
                      className="w-full flex items-center gap-4 px-6 py-4 hover:bg-gray-50 transition text-left"
                    >
                      ...
                      <div className="flex-1">
                        <p className="font-semibold text-gray-900 text-sm">{dateKey}</p>
                        <p className="text-xs text-gray-400 mt-0.5">
                          {txs.length} transaction{txs.length !== 1 ? "s" : ""}
                          {totalAmount !== 0 && <> · {formatSGD(totalAmount)}</>}
                        </p>
                      </div>
                      <button
                        onClick={(e) => { e.stopPropagation(); setConfirmDeleteId(`email-date:${dateKey}`); }}
                        disabled={!!deleting}
                    ...
                    {isOpen && (
                      <div className="border-t border-gray-50 px-6 py-4 space-y-2">
                        {txs.map((tx) => (
                          <div key={tx.id} className="flex items-center justify-between bg-gray-50 rounded-xl px-4 py-3 gap-3">
                            <div className="flex-1 min-w-0">
                              <p className="text-sm font-medium text-gray-800 truncate">
                                {tx.merchant || tx.source || tx.subject || "Unknown"}
                              </p>
                              <div className="flex items-center gap-2 mt-1 flex-wrap">
                                ...
                                {tx.from     && <span className="text-xs text-gray-400 truncate max-w-40">{tx.from}</span>}
                              </div>
                            </div>
                            <span className={`text-sm font-semibold shrink-0 ${(Number(tx.amount) || 0) >= 0 ? "text-emerald-600" : "text-red-500"}`}>
        ...
        <section className="bg-white border border-gray-100 rounded-2xl p-6">
          ...
          <p className="text-sm text-gray-600 leading-relaxed">
            Uploaded PDFs and CSV files are parsed to extract balances, transactions and asset
            values. Our system analyzes these files to generate portfolio insights and financial
            visualizations. No banking credentials or direct bank connections are required.
          </p>
        </section>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-6 text-center">
            {[
              ...
              <>
                ...
                {i < arr.length - 1 && (
                  <ArrowRight key={`arrow-${i}`} className="text-gray-300 shrink-0" />
                )}
              </>
            ))}
          </div>
        ...
        <section className="bg-white border border-gray-100 rounded-2xl p-6">
          ...
          <div className="mt-8 pt-6 border-t flex flex-wrap gap-4 sm:gap-6">
            <button
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-600 mt-1">
                This will permanently delete <strong>all</strong> uploaded statements, email
                transactions, wellness scores, net worth history, and manual accounts. Your
                login and profile will be kept.
              </p>
              <button
        ...
        <section className="bg-white border border-gray-100 rounded-2xl p-6">
          ...
          <p className="text-sm text-gray-500 mb-5">
            If you have questions about file uploads, document parsing, or security,
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="flex gap-4 text-xs font-semibold text-[#2081C3]">
              ...
              <a href="/docs" target="_blank" className="hover:underline">View Docs</a>
            </div>
          </div>
        </section>
                ...
                <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
                  ...
                  <button
                    onClick={async () => {
                      ...
                      } else if (typeof confirmDeleteId === "string" && confirmDeleteId.startsWith("email-date:")) {
                        await deleteEmailTxByDate(confirmDeleteId.replace("email-date:", ""));
                      } else {
                  ...
                  >
                    ...
                    {confirmDeleteId === "reset-all"
                      ? "Reset everything"
                      : confirmDeleteId === "all" || confirmDeleteId === "all-email"
                        ? "Delete all"
                        : "Delete"
                    }
                  </button>
