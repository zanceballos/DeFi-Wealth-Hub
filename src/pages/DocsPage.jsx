...
const TECH_STACK = [
  {
    ...
    reason: "Fastest HMR, modern JSX transform, minimal config",
  },
  {
    layer: "Styling",
    tech: "Tailwind CSS 4",
    reason: "Utility-first, zero runtime, glass-morphism via backdrop-blur",
  },
  {
    layer: "Database",
    tech: "Cloud Firestore",
    reason: "Real-time, serverless, nested subcollections for transactions",
  },
  {
    layer: "Auth",
    tech: "Firebase Auth",
    reason:
      "Email + Google OAuth out of the box, integrates with Firestore rules",
  },
  {
    layer: "Storage",
    tech: "Firebase Storage",
    reason: "Secure file uploads with per-user path rules",
  },
  {
    layer: "AI Parsing",
    tech: "InternVL",
    reason:
      "Vision-language model for extracting structured data from statement PDFs",
  },
  {
    layer: "AI Advisory",
    tech: "Groq (Llama 3.3 70B)",
    reason:
      "Ultra-fast inference, structured JSON output, Singapore-aware financial prompting",
  },
  {
    layer: "Charts",
    tech: "Recharts 3.7",
    reason: "Declarative React charts — PieChart, LineChart, RadarChart",
  },
  {
    layer: "Icons",
    tech: "Lucide React",
    reason: "Consistent, tree-shakeable icon library",
  },
  {
    layer: "Routing",
    tech: "React Router 7",
    reason: "Nested layouts, protected route wrappers, NavLink active states",
  },
  {
    layer: "HTTP",
    tech: "Axios",
    reason: "Statement upload to parsing backend",
  },
];
...
const ROUTES = [
  ...
  {
    route: "/dashboard",
    page: "Dashboard (Overview/Budget/Wallet)",
    auth: true,
  },
...
function SectionTitle({ icon: Icon, title }) {
  return (
    <div className="flex items-center gap-3 mb-6">
      <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-white/5 text-teal-400">
...
function CodeBlock({ children }) {
  return (
    <pre className="overflow-x-auto rounded-xl border border-white/10 bg-black/40 p-4 text-xs leading-relaxed text-teal-200">
      <code>{children}</code>
    </pre>
  );
}
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-white/10 text-left">
            {headers.map((h) => (
              <th key={h} className="pb-3 pr-4 font-semibold text-slate-300">
                {h}
              </th>
            ))}
          </tr>
          ...
          {rows.map((row, i) => (
            <tr key={i}>
              {row.map((cell, j) => (
                <td
                  key={j}
                  className={`py-2.5 pr-4 ${j === 0 ? "font-mono text-teal-300 text-xs" : "text-slate-300"}`}
                  style={
                    j > 0 && j === row.length - 1 ? { color: "#8899AA" } : {}
                  }
                >
                  {cell}
                </td>
...
export default function DocsPage() {
  ...
  // Observer for mid-page sections
  useEffect(() => {
  ...
  // Scroll fallback for short bottom sections
  useEffect(() => {
    function handleScroll() {
      ...
      if (sectionEntries.length > 0) {
        setActiveSection(sectionEntries[0].id);
      }
    }
  ...
  return (
    <div className="min-h-screen" style={{ background: "#0A0F1E" }}>
      {/* Top bar */}
      <header
        className="sticky top-0 z-40 flex items-center justify-between border-b border-white/10 px-6 py-3 backdrop-blur-xl"
        style={{ background: "rgba(10,15,30,0.85)" }}
      >
        <div className="flex items-center gap-3">
          <button
            className="mr-1 rounded-lg p-1.5 text-slate-400 hover:bg-white/10 lg:hidden"
          ...
          >
            ...
            <span className="font-normal text-slate-400">Docs</span>
          </Link>
        </div>
        <a
      ...
      <div className="mx-auto flex max-w-screen-xl">
        {/* Sidebar — desktop */}
        <aside className="sticky top-14 hidden h-[calc(100vh-3.5rem)] w-64 shrink-0 overflow-y-auto border-r border-white/10 px-4 py-8 lg:block">
        ...
        {mobileNavOpen && (
          ...
          >
            <div
              className="absolute left-0 top-14 w-64 border-r border-white/10 px-4 py-8 backdrop-blur-xl"
              style={{ background: "rgba(10,15,30,0.97)", bottom: 0 }}
              ...
              <nav className="space-y-0.5">
                {NAV_SECTIONS.map(({ id, label, icon: Icon }) => (
                  <button
                    key={id}
                    onClick={() => scrollTo(id)}
                    className="flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-sm font-medium text-slate-400 hover:bg-white/5 hover:text-white transition"
                  >
                    <Icon className="h-4 w-4 shrink-0" />
                    {label}
                  </button>
        ...
        <main className="min-w-0 flex-1 px-6 py-12 lg:px-12">
          <div className="mx-auto max-w-3xl space-y-20">
            ...
            <section id="overview" ref={setRef("overview")}>
              <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-white/10 px-4 py-1.5 text-xs font-medium uppercase tracking-wider text-teal-300">
              ...
              <p
                ...
                style={{ color: "#8899AA" }}
              >
                A privacy-first AI financial cockpit that helps users upload,
                parse, understand, and act on their financial data across
                banking, crypto, and investments — without ever sharing
                passwords or connecting bank accounts.
              </p>
            ...
            <section id="the-problem" ref={setRef("the-problem")}>
              <SectionTitle icon={Lightbulb} title="The Problem" />
              <p className="mb-6 leading-relaxed" style={{ color: "#8899AA" }}>
                Personal finance data is fragmented. Users juggle bank
                statements, crypto exchange reports, and investment summaries
                across multiple platforms. Existing solutions fall short:
              </p>
              <Table
                headers={["Existing Tool", "The Gap"]}
                rows={[
                  [
                    "Mint, Plaid-based apps",
                    "Force you to hand over bank login credentials",
                  ],
                  [
                    "Crypto portfolio trackers",
                    "Only cover digital assets — no TradFi",
                  ],
                  [
                    "Spreadsheet templates",
                    "Manual, error-prone, no intelligence layer",
                  ],
                  [
                    "Robo-advisors",
                    "Black-box recommendations, zero transparency",
                  ],
                ]}
              />
            </section>
            ...
            <section id="our-solution" ref={setRef("our-solution")}>
              <SectionTitle icon={CheckCircle} title="Our Solution" />
              <p className="mb-4 leading-relaxed" style={{ color: "#8899AA" }}>
                DeFi Wealth Hub takes a fundamentally different approach —{" "}
                <strong className="text-white">
                  deliberate, user-controlled ingestion
                </strong>
                :
              </p>
              <CodeBlock>{`┌──────────────┐     ┌────────────────┐     ┌──────────────────┐     ┌────────────────┐
...
└──────────────┘     └────────────────┘     └──────────────────┘     └────────────────┘
              ...
              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                {[
                  {
                    icon: Lock,
                    text: "No bank credentials required — upload statements on your own terms",
                  },
                  {
                    icon: Users,
                    text: "Human-in-the-loop — review, edit, or reject every parsed row",
                  },
                  {
                    icon: Wallet,
                    text: "TradFi + DeFi in one place — bank, broker, crypto, investments unified",
                  },
                  {
                    icon: Brain,
                    text: "Explainable AI — every insight links back to specific data points",
                  },
                  {
                    icon: ShieldCheck,
                    text: "Privacy by design — you always see exactly what data is stored and how it's used",
                  },
                ].map(({ icon: Icon, text }) => (
                  <Card key={text} className="flex items-start gap-3">
            ...
            <section id="features" ref={setRef("features")}>
              <SectionTitle icon={Sparkles} title="Features" />
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    >
                      ...
                      All data is live from Firestore — every upload refreshes
                      your numbers in real time.
                    </p>
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-white/10 text-left">
                          <th className="pb-2 font-semibold text-slate-300">
                            Tab
                          </th>
                          <th className="pb-2 font-semibold text-slate-300">
                            What It Shows
                      ...
                      <tbody className="divide-y divide-white/5">
                        {[
                          [
                            "Overview",
                            "Net worth history chart, overall wellness score, four financial health pillars (Liquidity, Diversification, Risk Match, Digital Health), savings rate, hero stat cards",
                          ],
                          [
                            "Budgeting",
                            "Category-based spending breakdown with progress bars, inflow vs. outflow analysis, monthly budget tracking, recent transactions with merchant detection",
                          ],
                          [
                            "Wallet",
                            "Platform-level asset allocation pie chart, per-platform rows with risk labels (Core / Stable / Growth / Speculative), regulated vs. unregulated exposure",
                          ],
                        ].map(([tab, desc]) => (
                          <tr key={tab}>
                            <td className="py-2 pr-4 font-medium text-teal-300 whitespace-nowrap">
                              {tab}
                            </td>
                            <td
              ...
              <Card className="mb-4">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    >
                      A dedicated{" "}
                      <code className="text-teal-300">/advisory</code> page
                      powered by{" "}
                      <strong className="text-white">
                        Groq AI (Llama 3.3 70B)
                      </strong>{" "}
                      that turns raw financial data into structured, actionable
                      guidance.
                    </p>
                    <CodeBlock>
                      {
                    ...
                    <p className="mt-3 rounded-lg border border-teal-500/20 bg-teal-500/10 px-3 py-2 text-xs text-teal-300">
                      ⚡ Rate-limit aware — results cached in{" "}
                      <code>sessionStorage</code> so the LLM is called only once
                      per browser session.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p
                      ...
                      style={{ color: "#8899AA" }}
                    >
                      A robust, multi-step document workflow that keeps the
                      human in control at every stage.
                    </p>
...
└── services/
    ...
    └── statementIngestionService.js  # Statement + transaction subcollection writer`}</CodeBlock>
              ...
              <p className="mt-6 mb-3 font-semibold text-white">Data Flow</p>
    ...
    match /users/{uid}/{allPaths=**} {
      allow read, write: if request.auth != null
            ...
            <section id="data-model" ref={setRef("data-model")}>
              <SectionTitle icon={Building2} title="Firestore Data Model" />
              <CodeBlock>{`/users/{uid}
...
└── /history/net_worth/items/{monthKey}
    ...
    └── updated_at   (timestamp)`}</CodeBlock>
            ...
            <section id="aggregation" ref={setRef("aggregation")}>
              <SectionTitle icon={Calculator} title="Aggregation & Metrics" />
              <p
                className="mb-4 text-sm leading-relaxed"
                style={{ color: "#8899AA" }}
              >
                Before pillar scores are computed, these values are aggregated
                from all <strong className="text-white">active</strong>{" "}
                (parsed/approved) statements:
              </p>
              <Table
                ...
                rows={[
                  [
                    "totalNetWorth",
                    "Σ stmt.net_worth_contribution",
                    "S$86,700",
                  ],
                  [
                    "totalCash",
                    "Σ stmt.asset_class_breakdown.cash",
                    "S$32,100",
                  ],
                  [
                    "totalCrypto",
                    "Σ stmt.asset_class_breakdown.crypto",
                    "S$12,800",
                  ],
                  [
                    "totalTokenised",
                    "Σ stmt.asset_class_breakdown.tokenised",
                    "S$4,200",
                  ],
                  [
                    "totalUnregulated",
                    "Σ net_worth_contribution where source_type ∈ {crypto, other}",
                    "S$12,800",
                  ],
                  [
                    "largestContribution",
                    "max(stmt.net_worth_contribution)",
                    "S$39,200",
                  ],
                ]}
              />
              ...
              <Table
                ...
                rows={[
                  [
                    "cashBufferMonths",
                    "totalCash / monthlyExpenses",
                    "32100 / 3458 = 9.28 months",
                  ],
                  [
                    "cryptoPct",
                    "(totalCrypto / totalNetWorth) × 100",
                    "14.76%",
                  ],
                  [
                    "digitalPct",
                    "((totalCrypto + totalTokenised) / totalNetWorth) × 100",
                    "19.61%",
                  ],
                  [
                    "unregulatedPct",
                    "(totalUnregulated / totalNetWorth) × 100",
                    "14.76%",
                  ],
                  [
                    "largestPositionPct",
                    "(largestContribution / totalNetWorth) × 100",
                    "45.21%",
                  ],
                  [
                    "savingsRatePct",
            ...
            <section id="scoring" ref={setRef("scoring")}>
              <SectionTitle icon={Brain} title="Scoring Formulas" />
              <p
                className="mb-4 text-sm leading-relaxed"
                style={{ color: "#8899AA" }}
              >
                ...
                <code className="text-teal-300">
                  clampScore(x) = round(min(100, max(0, x)))
                </code>
                .
              </p>
                    ...
                    table: {
                      ...
                      rows: [
                        [
                          "0%",
                          "70 🟢",
                          "No digital = safe but not leveraging Web3",
                        ],
                        ["15%", "85 🟢", "Sweet spot"],
                        ...
                        <thead>
                          <tr className="border-b border-white/10 text-left">
                            {table.headers.map((h) => (
                              <th
                                key={h}
                                className="pb-2 pr-4 font-semibold text-slate-400"
                              >
                                {h}
                              </th>
                            ))}
                          ...
                          {table.rows.map((row, i) => (
                            <tr key={i}>
                              {row.map((cell, j) => (
                                <td
                                  key={j}
                                  className={`py-1.5 pr-4 ${j === 0 ? "text-teal-300 font-mono" : "text-slate-300"}`}
                                >
                                  {cell}
                                </td>
                              ))}
              ...
              <Card className="mt-4">
                ...
                <CodeBlock>
                  {
                    "overallScore = round((liquidity + diversification + riskMatch + digitalHealth) / 4)"
                  }
                </CodeBlock>
                <table className="mt-3 w-full text-xs">
                  <tbody className="divide-y divide-white/5">
                    {[
                      ...
                      <tr key={range}>
                        <td className="py-1.5 pr-4 font-mono text-teal-300">
                          {range}
                        </td>
            ...
            <section id="budget-calc" ref={setRef("budget-calc")}>
              <SectionTitle icon={TrendingUp} title="Budget Calculations" />

              <p className="mb-4 font-semibold text-white text-sm">
                Budget & Emergency Savings
              </p>
              <Table
                headers={["Metric", "Formula", "Example"]}
                rows={[
                  ...
                  [
                    "spentThisMonth",
                    "Σ stmt.parsed_data.total_debits (current month)",
                    "S$2,100",
                  ],
                  [
                    "remainingBudget",
                  ...
                  [
                    "emergencySavingsCurrent",
                    "totalCash + totalBonds (capped at 1.5× target)",
                    "S$32,100",
                  ],
                  [
                    "emergencySavingsPct",
              ...
              <p className="mt-6 mb-4 font-semibold text-white text-sm">
                Wallet Allocation
              </p>
              <Card>
                <p className="mb-2 text-sm text-slate-300">
                  For each asset class in{" "}
                  <code className="text-teal-300">
                    [cash, bonds, stocks, crypto, property, tokenised]
                  </code>
                  :
                </p>
                <CodeBlock>
                ...
                <p className="mt-3 mb-2 text-sm text-slate-300">
                  Per-platform row:
                </p>
                <CodeBlock>
                  {
              ...
              <Card>
                ...
                <ol className="space-y-1 text-sm" style={{ color: "#8899AA" }}>
                  {[
            ...
            <section id="worked-example" ref={setRef("worked-example")}>
              ...
              >
                A user with{" "}
                <strong className="text-white">monthly income S$4,490</strong>{" "}
                and{" "}
                <strong className="text-white">monthly expenses S$3,458</strong>{" "}
                uploads 3 statements:
              </p>
              ...
              <p className="mt-6 mb-2 font-semibold text-white text-sm">
                Step 1 — Aggregate
              </p>
...
largestContribution= S$32,100  (DBS)`}</CodeBlock>
              ...
              <p className="mt-4 mb-2 font-semibold text-white text-sm">
                Step 2 — Percentages
              </p>
...
savingsRatePct     = (4,490 − 3,458) / 4,490 × 100 = 22.98%`}</CodeBlock>
            ...
            <section id="tech-stack" ref={setRef("tech-stack")}>
              ...
              <Card>
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-white/10 text-left">
                      <th className="pb-3 pr-3 font-semibold text-slate-300">
                        Layer
                      </th>
                      <th className="pb-3 pr-3 font-semibold text-slate-300">
                        Technology
                      </th>
                      <th className="pb-3 hidden sm:table-cell font-semibold text-slate-300">
                        Why
                      </th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-white/5">
                    {TECH_STACK.map(({ layer, tech, reason }) => (
                      <tr key={layer}>
                        ...
                        <td
                          className="py-2.5 hidden sm:table-cell text-xs"
                          style={{ color: "#8899AA" }}
                        >
                          {reason}
                        </td>
            ...
            <section id="getting-started" ref={setRef("getting-started")}>
              <SectionTitle icon={Rocket} title="Getting Started" />
              <p className="mb-4 text-sm" style={{ color: "#8899AA" }}>
                Prerequisites: Node.js ≥ 18, npm ≥ 9, a Firebase project with
                Auth + Firestore + Storage enabled, and a Groq API key (free
                tier at{" "}
                <a
...
cp .env.local.example .env.local`}</CodeBlock>
              ...
              <p className="mt-5 mb-2 font-semibold text-white text-sm">
                Add to <code className="text-teal-300">.env.local</code>
              </p>
    ...
    match /users/{uid}/{document=**} {
      allow read, write: if request.auth != null && request.auth.uid == uid;
            ...
            <section id="routes" ref={setRef("routes")}>
              <SectionTitle icon={Map} title="Pages & Routes" />
              <Card>
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-white/10 text-left">
                  ...
                  <tbody className="divide-y divide-white/5">
                    {ROUTES.map(({ route, page, auth }) => (
                      <tr key={route}>
                        <td className="py-2.5 pr-4 font-mono text-teal-300 text-xs">
                          {route}
                        </td>
                        <td className="py-2.5 pr-4 text-slate-200">{page}</td>
                        <td className="py-2.5">
                          <span
                            className={`rounded-full px-2 py-0.5 text-xs font-medium ${auth ? "bg-teal-500/10 text-teal-300" : "bg-slate-500/10 text-slate-400"}`}
                          >
            ...
            <section id="contributing" ref={setRef("contributing")}>
              <SectionTitle icon={GitBranch} title="Contributing" />
                ...
                {[
                  ...
                  <li key={i} className="flex items-start gap-3">
                    <span
                      ...
                      style={{
                        background: "linear-gradient(135deg,#00C9B1,#0099FF)",
                      }}
                    >
                      {i + 1}
                    </span>
            ...
            <section id="license" ref={setRef("license")}>
              <SectionTitle icon={FileText} title="License" />
              <Card>
                <p
                ...
                >
                  ...
                  . Please contact the team for licensing enquiries.
                </p>
              </Card>
            </section>
            ...
            <section id="team" ref={setRef("team")}>
              <SectionTitle icon={Users} title="The Team" />
              <p className="mb-4 text-sm" style={{ color: "#8899AA" }}>
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {TEAM.map(({ name, major, github }) => (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="group flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 p-4 transition hover:border-teal-500/40 hover:bg-white/10"
                  >
                    <div
                    ...
                    <Github className="ml-auto h-4 w-4 text-slate-600 group-hover:text-slate-400 transition" />
                  </a>
                ))}
              </div>
              ...
              >
                Built with ❤️ at NTU ·{" "}
                <em>
                  Where traditional finance meets decentralised innovation
                </em>
              </p>
            </section>
          </div>
